...
export function makePushHandler({ db }) {
  ...
  // Pre-check probe — indexed lookup on the UNIQUE constraint. Returns null
  // if the row is new (so we should count it as accepted); a 1 if we already
  // had it (deduplicated).
      ...
      .then((body) => {
        ...
        const tx = db.transaction(() => {
          for (const row of payload.rows) {
            ...
            // applyRow returns false on hard validation failure; we just skip
            // it. Soft errors (e.g. constraint surprise) bump deduplicated.
            void ok;
          }
        });
        ...
        const last_id = lastRow?.id ?? 0;
        const body200 = { accepted, deduplicated, last_id };
        if (firstError) body200.warning = firstError;
