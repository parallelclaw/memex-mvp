...
npx -y clawhub skill publish skills/install-memex-claw \
  ...
  --clawscan-note "Skill installs two open-source MIT packages: (1) memex-mvp (npmjs.com/package/memex-mvp, source github.com/parallelclaw/memex-mvp) via 'npm install -g memex-mvp@latest' — provides memex CLI and MCP server. (2) @parallelclaw/memex-openclaw (npmjs.com/package/@parallelclaw/memex-openclaw, source github.com/parallelclaw/memex-mvp/tree/main/plugins/memex-openclaw) via 'openclaw plugins install @parallelclaw/memex-openclaw' — provides the OpenClaw plugin that captures turns via the native plugin SDK. If an OLD v0.11.x memex-sync daemon is running (systemd/LaunchAgent), the skill stops and disables it because it would conflict with the plugin's capture path; sudo is never used. Edits the OpenClaw gateway config at ~/.openclaw/openclaw.json to (a) enable the memex-openclaw plugin under plugins.entries and (b) add a 'memex' MCP server entry under mcpServers — both merge-not-overwrite to preserve other plugins/MCP servers. memex stores entirely locally in ~/.memex/data/memex.db. Zero outbound network traffic at runtime. All commands shown to user before execution. Plugin source is fully readable JS in plugins/memex-openclaw/."
