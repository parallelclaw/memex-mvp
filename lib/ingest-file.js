...
async function ingestClaudeJsonl(db, path, format, _opts) {
  const source =
    ...
    : 'claude-code';
  let fileName = basename(path, '.jsonl');
  // v0.10.17: OpenClaw checkpoint files (containing e.g. Telegram messages
  // received while the agent was busy on the main session) have inbox names
  // like `openclaw-<base8>-ckpt-<chkpt8>.jsonl`. Strip the `-ckpt-…` suffix
  // so checkpoint content lands in the SAME conversation_id as the main
  // session. Otherwise each checkpoint snapshot would create a parallel
  // conv and the user's Telegram-while-busy messages would float in their
  // own thread, disconnected from the live chat they're responding to.
  if (source === 'openclaw') {
