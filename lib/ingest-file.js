...
async function ingestOpenclawJsonl(db, path, _opts) {
  ...
  // Per-conversation stats — title / first_ts / last_ts. Computed during
  // ingest, applied once via upsertConversation at the end.
  const convStats = new Map();
  function bump(convId, ts, candidateTitle) {
    let s = convStats.get(convId);
    if (!s) {
      s = { first_ts: Infinity, last_ts: 0, title: null };
  ...
  // Running pointer to the channel + conv of the most recent USER message.
  // Assistant messages have no channel marker — they inherit from the user
  // they are responding to. For a Kimi-web session this means assistant
  // replies join the kimi-web conv; for Telegram, the most recent sender's
  // conv. Falls back to file-level (sessions.json) if a file starts with an
  // assistant message (rare).
  let lastUserChannel = fileFallbackChannel;
  let lastUserConvId = null;
  ...
  const tx = db.transaction(() => {
    for (const line of lines) {
      ...
      // Assistant messages: inherit channel + conv from the last user
      // message in the same file. detectChannel on assistant text would
      // return null (no marker), which we don't want.
      const channel = m.role === 'assistant'
        ? lastUserChannel
        : detectChannel(m.text, fileFallbackChannel);

      // Batched Telegram — one OpenClaw record → multiple memex messages
      if (channel === 'telegram' && /^\[Queued messages while agent was busy\]/i.test(m.text)) {
        const batches = parseBatchedTelegram(m.text);
        for (const b of batches) {
          if (!b.sender_id) continue; // can't route without sender
          const convId = deriveOpenclawConvId('telegram', b.sender_id, fileUuid8);
          lastUserChannel = 'telegram';
          ...
          try {
            const r = insertMessage.run(
              ...
              b.sender || (b.username ? `@${b.username}` : 'telegram'),
              b.text || '', ts, metadata, null,
              null, // no uuid for batched TG — OpenClaw doesn't expose one
              'telegram',
            );
            if (r.changes > 0) imported++;
            bump(convId, ts, b.text);
          } catch (_) { /* dupe via UNIQUE */ }
        }
        continue;
      }

      // Single Telegram (not batched — happens when agent was free and
      // a TG message arrives directly into the main session file)
      if (channel === 'telegram' && m.role === 'user') {
        const parsed = parseSingleTelegram(m.text);
        if (parsed && parsed.sender_id) {
          const convId = deriveOpenclawConvId('telegram', parsed.sender_id, fileUuid8);
          lastUserChannel = 'telegram';
          lastUserConvId = convId;
          ...
          try {
            const r = insertMessage.run(
              source, convId, msgId, m.role,
              parsed.sender || (parsed.username ? `@${parsed.username}` : 'telegram'),
              parsed.text || '', ts, metadata, null,
              m.uuid || null,
              'telegram',
            );
            if (r.changes > 0) imported++;
            bump(convId, ts, parsed.text);
          } catch (_) { /* dupe */ }
          continue;
        }
        // Failed to parse single-TG metadata — fall through to default
      }

      // Default path: kimi-web / system / null channel
      // Assistants reuse the last user's conv to stay glued to the thread
      // (e.g. assistant reply in a Kimi session → openclaw-kimi-<file8>,
      // assistant reply in a TG session → last sender's openclaw-tg-<id>).
      const convId = m.role === 'assistant' && lastUserConvId
        ? lastUserConvId
        : deriveOpenclawConvId(channel, null, fileUuid8);
      const text = channel === 'kimi-web' && m.role === 'user'
        ? stripKimiHeader(m.text)
        : m.text;
      if (!text || !text.trim()) continue;
      const sender = m.role === 'user' ? 'me' : source;
      const msgId = m.id || obj.id || `openclaw-${fileName}-${baseTs}`;
      // Update lastUser pointer for user-role records on the default path
      // (kimi-web user, system user). Assistant records do NOT advance it.
      if (m.role === 'user') {
      ...
      try {
        ...
        if (r.changes > 0) imported++;
        bump(convId, baseTs, m.role === 'user' ? text : null);
      } catch (_) { /* dupe */ }
    }

    // Upsert all conversations seen in this file
    for (const [convId, s] of convStats.entries()) {
      const channelTag = convId.startsWith('openclaw-tg-')
        ? '[Telegram]'
        : convId.startsWith('openclaw-kimi-')
        ? '[Kimi-web]'
        : convId.startsWith('openclaw-sys-')
        ? '[System]'
        : '';
      const titleText = s.title || fileName;
      const title = channelTag ? `${channelTag} ${titleText}` : titleText;
      upsertConversation.run(
        convId, source, title,
